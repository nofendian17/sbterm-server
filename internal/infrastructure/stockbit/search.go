...
// SearchResponse is the symbol search response. Sections the upstream did not
// query for are present as empty slices, not null.
type SearchResponse struct {
	Message string      `json:"message"`
	Data    SearchData  `json:"data"`
	Errors  []SearchErr `json:"errors,omitempty"`
}
...
// GetSearch searches symbols, insiders, people, sectors and industries.
// keyword selects the query, page the 1-based page number, and typ the section
// to search (company, insider, people, sector, industries, chat, all) — the
// upstream treats every other value as "all" and returns every section. The
// access token is attached automatically.
func (c *Client) GetSearch(ctx context.Context, keyword string, page int, typ string) (*SearchResponse, error) {
	q := url.Values{}
	...
	if err := c.Get(ctx, searchPath, q, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
